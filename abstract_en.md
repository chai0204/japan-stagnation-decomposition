...
**Theoretical contributions (calibration-based simulations)**: We develop and integrate (i) a formal two-sector small open economy model, (ii) a DSGE-light model with capital adjustment costs and dynamic transitions, (iii) a HANK-light model with four heterogeneous household types, and (iv) a fully integrated HANK+DSGE model. The combined ICT-investment-plus-wage-share reform produces a central simulation of lifetime CE welfare gains of +25.8%, with inequality (Gini-like) declining 44% (95% of population gains; 5% — self-employed/owners — face transition losses requiring policy compensation). **These results are calibration-based simulations, not structurally estimated; the sensitivity range across model variants is approximately [+8%, +55%]**.

**Policy implications**: Service-sector productivity reform paired with wage-share restoration, supported by transitional compensation for capital owners, may contribute to reversing the visible "Lost 30 Years" pattern over a 10–20 year horizon. **Identification of these channels relies on observational data; causal claims (especially H6) should be interpreted as suggestive rather than definitive — see Section 7.5 for limitations**.

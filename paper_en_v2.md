...
The **primary contribution is single**: identifying the wage-productivity wedge in a G7+Korea cross-country panel as a **constant level penalty rather than a slope failure**, and demonstrating that its mechanism is non-identifiable in observable cross-country macro covariates.
...
- H8 is identified as a "constant level penalty," not a "transmission failure"
...
This is the **central theorem** of this paper. Japanese data 1995–2024 shows:
...
|---|---|---|
| **Constant level penalty (β = −2.15 pp/yr)** | Wage-productivity divergence (Stansbury-Summers 2018) | Extension of US findings to G7+Korea, refinement via slope test |
...
**H8 structural characterization**: "Japan's wage-productivity transmission works, but a constant level penalty applies":
...
Then what is the real problem?
    ...
    ↓
Is H8 "transmission failure" or "level penalty"?
    ↓ Interaction model identifies: slope intact, level penalty
    ↓
What is the mechanism behind level penalty?
...
This is **NOT a "Japan is not stagnating" paper**. The core message is: **"Japan's wage transmission works, but a constant level penalty stems from a mechanism unobservable in macro data."**
...
### 5.2 Aggregation level reorganization (prerequisite, H1 measurement reinterpretation)

> **Note**: This subsection provides H1 (per-WA equality) prerequisite verification. It is NOT an independent contribution. Positioned as a replication of Fernández-Villaverde et al. (2024), required to show §5.1's puzzle is **not driven by demographic aggregation effects**.

Comparing 4 aggregation levels:
...
**Finding 2 (central)**: But Japan's **wages are constantly down-shifted by −2.15 pp/yr regardless of productivity level**. Identified as "constant level penalty."

**Finding 3 (mechanism)**: This level penalty is **not attenuated by any of 4 observable mediators** (labor share, self-employment, vulnerable employment, aging) (§6.6). **Mechanism lies outside cross-country macro variables.**
...
> Or is the slope the same but a constant level penalty? (level shift)
...
→ **H8 is identified as a constant level shift (intercept shift)**. Slope is identical, but a constant −2.15 pp/yr level penalty applies independent of productivity level.
...
§6.1–§6.5 identified "Japan's wage-productivity wedge is robust as a constant level penalty." This section **attempts mechanism identification**: sequentially adding observable mediators, testing whether Japan dummy attenuates.
...
#### 6.6.5 What this means

In all 11 mediator specifications, Japan dummy |β| ≥ 1.91, **all p<0.001**. Adding 4 observable macro covariates does not erase Japan-specificity.

This is consistent with two interpretations:

1. **H8 mechanism is constant (time-invariant)**: Japan-specific structure applies as level offset, time-varying mediators cannot capture. Consistent with §6.1.5 (slope intact, level shift only).
2. **Observable macro proxies are insufficient**: Japan-specific institutional features (Japanese non-regular employment expansion, internal labor markets, retained earnings preferences) don't appear in cross-country macro data. **Identification requires micro data** (corporate financial statistics, labor structure surveys).

#### 6.6.6 Implication: bridging to Paper 2

This paper's primary contribution (H8 = constant level penalty) is established, but **its mechanism identification is impossible in cross-country panel**. This is Paper 2's independent research:
...
→ This paper's "negative result" (mediators non-identifiable) is itself a contribution: **empirically establishing Paper 2's necessity**.
...
- Japan has comparable slope (+0.62) but constant level penalty of −2.15
